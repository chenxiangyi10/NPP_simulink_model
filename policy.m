...
W0 = 680.4; % [kg/s] initial steam mass flow rate
...
h_fw = h_sg_out0 - P_thermal0 * 1e3 / num_sg / W0; % [kJ/kg] feedwater specific enthalpy
...
Pt0 = (h_sg_out0 - h_cd0) * eta * W0 *1e-3 *2 ; % 846 [MW] the turbine initial power
...
T_c_h_diff_policy = P_thermal_policy / M;
T_p_policy = P_thermal_policy / Z1 + P_thermal_policy / Z2 + T_sg_out0;
T_c_policy = T_p_policy - T_c_h_diff_policy / 2;
T_h_policy = T_p_policy + T_c_h_diff_policy / 2; 


plot(P_thermal_policy / P_thermal0 * num_sg, T_h_policy, 'red')
hold on
plot(P_thermal_policy / P_thermal0 * num_sg, T_p_policy, 'm')
hold on
